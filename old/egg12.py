...
т. е. внутри A есть тег <a href="B">,
...
что из A в C можно перейти за один переход и из C в B
...
first_res = requests.get(first)
pattern_href = r'<a [\w\s]*href="(.*)">'

hrefs = []
for i in re.findall(pattern_href, first_res.text):
    second_res = requests.get(i)
    hrefs.extend(re.findall(pattern_href, second_res.text))
