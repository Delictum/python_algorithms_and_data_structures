...
def binary_search(bin_search_tree, number, path=''):
    ...
    if number < bin_search_tree.value and bin_search_tree.left != None:
        return binary_search(bin_search_tree.left, number, path=f'{path}\nШаг влево')

    if number > bin_search_tree.value and bin_search_tree.right != None:
        return binary_search(bin_search_tree.right, number, path=f'{path}\nШаг вправо')

    return f'Число {number} отсутствует в дереве'


bt = bst(height=4, is_perfect=False)
print(bt)

num = int(input('Введите число для поиска: '))
print(binary_search(bt, num))
